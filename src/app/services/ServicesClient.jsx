...
const bespokeOffers = [
  ...
  },
  {
    ...
    package: 'Package 2',
    title: 'Research',
    description: 'Use various research methods to define user needs and business product objectives.',
    details: [],
    goodFor:
  ...
  },
  {
    ...
    details: [],
    goodFor:
      'Anyone who has done the user research, built a business and business strategy, understand they need designing our comprehensive solutions and want to test and iterate these solutions in prototyping.',
  },
  {
    id: 5,
    package: 'Package 5',
    ...
    <div className="min-h-screen bg-[#121212] text-white">
      ...
      >
        <motion.div variants={itemVariants} className="text-center max-w-6xl mx-auto">
          ...
          >
            ...
            solve real-world problems through research and thoughtful implementation.
          </motion.p>
        </motion.div>
      </motion.section>
      ...
      >
        <div className="max-w-6xl mx-auto">
          <motion.div className="text-center mb-2 sm:mb-12" variants={containerVariants}>
            <motion.h2
            ...
            <motion.p className="font-body text-lg font-light text-neutral-300 leading-relaxed" variants={itemVariants}>
              I go into more detail of each step below in my various offerings
            </motion.p>
            <div className="w-24 h-1 bg-gradient-to-r from-neutral-600 to-neutral-500 mx-auto rounded-full mt-3" />
          </motion.div>

          <div className="flex justify-center">
            <Image
              src="/digital_page_top_icon.svg"
              alt="UI/UX design process — Double Diamond framework illustration"
              width={1000}
              height={420}
      ...
      <motion.section
        className="py-6 sm:py-20 px-4 bg-neutral-950/50"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
        variants={containerVariants}
      >
        <div className="max-w-6xl mx-auto">
          <motion.div className="text-center mb-12" variants={containerVariants}>
            ...
            <motion.p className="font-body text-lg font-light text-neutral-300 leading-relaxed max-w-4xl mx-auto mb-3" variants={itemVariants}>
              In this free discovery call, we can discuss precise price quotes and the best way
              ForwardFast would suit your needs.
            </motion.p>
            <div className="w-24 h-1 bg-gradient-to-r from-neutral-600 to-neutral-500 mx-auto rounded-full" />
          </motion.div>
              ...
              {bespokeOffers.map((offer, index) => {
                ...
                if (isBlackCard) {
                  return (
                    <motion.div
                      ...
                      className="relative bg-neutral-900/50 backdrop-blur-sm border border-neutral-800 rounded-2xl p-8 h-[300px] flex flex-col justify-center overflow-hidden"
                    >
                ...
                return (
                  <motion.div
                    ...
                    variants={itemVariants}
                    whileHover={{ scale: 1.02 }}
                    className="bg-white border border-neutral-300 rounded-2xl p-8 h-[300px] flex flex-col justify-center shadow-lg"
                  >
                    <h3 className="text-2xl font-heading font-semibold text-neutral-600 mb-4">{offer.title}</h3>
                    <p className="text-sm sm:text-base text-neutral-600 leading-relaxed mb-3">{offer.description}</p>
                    {offer.goodFor && (
                      <>
                        <h4 className="text-lg font-heading font-semibold text-neutral-600 mb-2">Good for:</h4>
                        <p className="text-sm sm:text-base text-neutral-600 leading-relaxed">{offer.goodFor}</p>
                      </>
                    )}
                  </motion.div>
                );
              ...
              {bespokeOffers.map((offer, index) => {
                ...
                if (isWhiteCard) {
                  return (
                    ...
                    >
                      <h3 className="text-2xl font-heading font-semibold text-neutral-600 mb-4">{offer.title}</h3>
                      <p className="text-base text-neutral-600 leading-relaxed mb-3">{offer.description}</p>
                      {offer.details.length > 0 && (
                        <>
                          <h4 className="text-xl font-heading font-semibold text-neutral-600 mb-2">Good for:</h4>
                          {offer.details.map((detail, i) => (
                            <div key={i} className="flex items-start mb-1">
                              <div className="w-2 h-2 bg-black rounded-full mt-2 mr-3 flex-shrink-0" />
                              <p className="text-base text-neutral-600 leading-relaxed">{detail}</p>
                            </div>
                          ))}
                        </>
                      )}
                      {offer.goodFor && (
                        <>
                          <h4 className="text-xl font-heading font-semibold text-neutral-600 mb-2">Good for:</h4>
                          <p className="text-base text-neutral-600 leading-relaxed">{offer.goodFor}</p>
                        </>
                      )}
                    </motion.div>
                  );
