...
async function optimizeImage(inputPath, outputDir, filename) {
  ...
  try {
    ...
    results.push({ format: 'WebP', path: webpPath, size: webpSizeMB > 1 ? `${webpSizeMB}MB` : `${webpSizeKB}KB` });
    ...
    if (ext === '.jpg' || ext === '.jpeg') {
      ...
      results.push({ format: 'JPEG', path: jpegPath, size: jpegSizeMB > 1 ? `${jpegSizeMB}MB` : `${jpegSizeKB}KB` });
    } else if (ext === '.png') {
      ...
      results.push({ format: 'PNG', path: pngPath, size: pngSizeMB > 1 ? `${pngSizeMB}MB` : `${pngSizeKB}KB` });
    }
...
async function scanAndOptimize() {
  ...
  try {
    ...
    const sizeSavings = ((totalOriginalSize - totalOptimizedSize) / totalOriginalSize * 100).toFixed(1);
    console.log(`\n💾 TOTAL SAVINGS: ${totalOriginalSize.toFixed(2)}MB → ${totalOptimizedSize.toFixed(2)}MB (${sizeSavings}% reduction)`);
